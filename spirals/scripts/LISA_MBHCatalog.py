...
# In the following block of code load the black hole and subfind properties saved above TODO
if read_subhalo_data is True:
    for region in regions:
        # Load the data saved in the read-in block of code. #
        times = np.load(save_data_path + 'times_' + str(region) + '.npy')
        ids_primary = np.load(save_data_path + 'ids_primary_' + str(region) + '.npy')
        ids_secondary = np.load(save_data_path + 'ids_secondary_' + str(region) + '.npy')
        masses_primary = np.load(save_data_path + 'masses_primary_' + str(region) + '.npy')
        ...
        for redshift in redshifts:
            # Avoid analysing non-existing redshifts. #
            try:
                ids = np.load(save_data_path + 'ids_' + str(region) + '_' + str(redshift) + '.npy')
            except FileNotFoundError:
                continue

            # Declare arrays to store the information from the 'eagle_subfind_particles_' text files. #
            galaxy_group_number, galaxy_subgroup_number = [], []

            # Load the data saved in the subfind block of code. #
            files = np.load(save_data_path + 'files_' + str(region) + '_' + str(redshift) + '.npy')
            masses = np.load(save_data_path + 'masses_' + str(region) + '_' + str(redshift) + '.npy')
            group_numbers = np.load(save_data_path + 'group_numbers_' + str(region) + '_' + str(redshift) + '.npy')
            subgroup_numbers = np.load(save_data_path + 'subgroup_numbers_' + str(region) + '_' + str(redshift)
                                       + '.npy')

            mask_redshift, = np.where(redshift == stellar_mass_redshifts)
            print(redshift, ids_primary[mask_redshift], ids_secondary[mask_redshift])
            exit()
            # Loop over all sets of black hole mergers for a given redshift. #
            for id_primary, id_secondary, mass_primary, mass_secondary in \
                    zip(ids_primary[mask_redshift], ids_secondary[mask_redshift], masses_primary[mask_redshift],
                        masses_secondary[mask_redshift]):

                # Mask the log file ids to match the subfind ids. #
                mask_primary, = np.where(id_primary == ids)
                mask_secondary, = np.where(id_secondary == ids)

                # If both black hole exist in the subfind files, select the most massive one. Else, select the one that
                # exists in the files. #
                if len(mask_primary) == 1 and len(mask_secondary) == 1:
                    if masses[mask_primary] >= masses[mask_secondary]:
                        galaxy_group_number.append(group_numbers[mask_primary][0])
                        galaxy_subgroup_number.append(subgroup_numbers[mask_primary][0])
                    elif masses[mask_primary] < masses[mask_secondary]:
                        galaxy_group_number.append(group_numbers[mask_secondary][0])
                        galaxy_subgroup_number.append(subgroup_numbers[mask_secondary][0])
                elif len(mask_primary) == 1 and len(mask_secondary) == 0:
                    galaxy_group_number.append(group_numbers[mask_primary][0])
                    galaxy_subgroup_number.append(subgroup_numbers[mask_primary][0])
                elif len(mask_secondary) == 1 and len(mask_primary) == 0:
                    galaxy_group_number.append(group_numbers[mask_secondary][0])
                    galaxy_subgroup_number.append(subgroup_numbers[mask_secondary][0])
                # elif len(mask_secondary) == 0 and len(mask_primary) == 0:

            if len(mask_redshift) - len(galaxy_group_number) > 0.0:
                print("Found", len(galaxy_group_number), "group and subgroup numbers at redshift", redshift,
                      "in region", region, "and missing", len(mask_redshift) - len(galaxy_group_number))

            np.save(save_data_path + 'galaxy_group_number_' + str(region) + '_' + str(redshift), galaxy_group_number)
            np.save(save_data_path + 'galaxy_subgroup_number_' + str(region) + '_' + str(redshift),
                    galaxy_subgroup_number)

# Mask particles whose group or subgroup numbers are 2e30 because they don't belong to any group or subgroup. #
# mask_invalid, = np.where((group_numbers >= 10 ** (30 * np.log10(2)))
#                          | (subgroup_numbers >= 10 ** (30 * np.log10(2))))
# group_numbers = np.delete(group_numbers, mask_invalid)
# Mask cases where the progenitor black hole belong in the same galaxy (i.e. same group and subgroup number). #
# array, indices = np.unique(group_numbers, return_index=True)
# group_numbers = group_numbers[indices]

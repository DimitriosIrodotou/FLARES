...
def plot_spiral_candidates():
    """
    Plot the V over sigma vs disc-to-total mass ratio of a given spiral candidate galaxy.
    :return: None
    """
    # Load the data. #
    all_group_numbers = np.load(data_path + 'group_numbers.npy')
    all_subgroup_numbers = np.load(data_path + 'subgroup_numbers.npy')
    glx_rotationals_over_dispersions = np.load(data_path + 'glx_rotationals_over_dispersions.npy', allow_pickle=True)
    ...
    if int(redshift) == 8:
        ...
        axis = axis03
    axis.scatter(glx_rotationals_over_dispersions[mask], glx_disc_fractions_IT20[mask], marker=get_hurricane(), s=1000,
                 lw=5,
...
def plot_barred_candidates():
    """
    Plot the V over sigma vs disc-to-total mass ratio of a given barred candidate galaxy.
    :return: None
    """
    # Load the data. #
    all_group_numbers = np.load(data_path + 'group_numbers.npy')
    ...
    axis.scatter(glx_rotationals_over_dispersions[mask], glx_disc_fractions_IT20[mask], marker=bar, s=1000, lw=5,
                 color='tab:red')
    return None
...
def plot_disc_dominated_galaxies():
    """
    Plot the V over sigma vs disc-to-total mass ratio of a disc-dominated candidate galaxy.
    :return: None
    """
    # Load the data. #
    try:
    ...
    for group_number, subgroup_number in zip(all_group_numbers[mask_disc_fraction],
        ...
        mask, = np.where((all_group_numbers == float(group_number)) & (all_subgroup_numbers == float(subgroup_number)))
        axis.scatter(glx_rotationals_over_dispersions[mask], glx_disc_fractions_IT20[mask], s=20, color='k', zorder=-5)


def plot_bulge_dominated_galaxies():
    """
    Plot the V over sigma vs disc-to-total mass ratio of a bulge-dominated candidate galaxy.
    :return: None
    """
    # Load the data. #
    try:
    ...
    for group_number, subgroup_number in zip(all_group_numbers[mask_disc_fraction],
        ...
        glx_rotationals_over_dispersions = np.load(data_path + 'glx_rotationals_over_dispersions.npy',
                                                   allow_pickle=True)
        mask, = np.where((all_group_numbers == float(group_number)) & (all_subgroup_numbers == float(subgroup_number)))
        axis.scatter(glx_rotationals_over_dispersions[mask], glx_disc_fractions_IT20[mask], s=20, color='tab:grey')
...
if __name__ == '__main__':
    ...
    for axis in axes:
        plot_tools.set_axes(axis, xlim=[0, 2.1], ylim=[0.5, 1], xlabel=r'$V / \sigma$', ylabel=r'$D/T$', which='major',
                            size=30)
        if axis != axis00:
            axis.set_ylabel('')
    ...
    plt.savefig('/cosma7/data/dp004/dc-irod1/FLARES/spirals/plots/VoverSigma_DoverT.png', bbox_inches='tight')
    plt.close()

...
def plot_spiral_candidates():
    """
    Plot the stellar vs gas/stellar mass ratio of a given spiral candidate galaxy.
    :return: None
    """
    # Load the data. #
    ...
    gaseous_data = np.load(data_path + 'gaseous_data_tmps/gaseous_data_tmp_' + str(group_number) + '_' +
                           str(subgroup_number) + '.npy', allow_pickle=True)
    gaseous_data = gaseous_data.item()
    ...
    axis.scatter(np.sum(stellar_data['Mass']),
                 np.sum(gaseous_data['Mass'][gaseous_data['star_forming_mask']]) / (
                         np.sum(stellar_data['Mass']) + np.sum(
                     gaseous_data['Mass'][gaseous_data['star_forming_mask']])),
                 marker=get_hurricane(), s=1000, lw=5, edgecolors='tab:blue', facecolors='none')
    return None


def plot_barred_candidates():
    """
    Plot the stellar vs gas/stellar mass ratio of a given barred candidate galaxy.
    :return: None
    """
    # Load the data. #
    all_group_numbers = np.load(data_path + 'group_numbers.npy')
    ...
    axis.scatter(np.sum(stellar_data['Mass']),
                 np.sum(gaseous_data['Mass'][gaseous_data['star_forming_mask']]) / (
                     ...
                     gaseous_data['Mass'][gaseous_data['star_forming_mask']])),
                 marker=bar,
                 s=1000, lw=5, color='tab:red')
    return None


def plot_disc_dominated_galaxies():
    """
    Plot the stellar vs gas/stellar mass ratio of a disc-dominated candidate galaxy.
    :return: None
    """
    # Load the data. #
    try:
    ...
    for group_number, subgroup_number in zip(all_group_numbers[mask_disc_fraction],
        ...
        axis.scatter(np.sum(stellar_data['Mass']),
                     np.sum(gaseous_data['Mass'][gaseous_data['star_forming_mask']]) / (
                                 np.sum(stellar_data['Mass']) + np.sum(
                             gaseous_data['Mass'][gaseous_data['star_forming_mask']])), s=20,
                     color='k', zorder=-5)
    return None
...
if __name__ == '__main__':
    ...
    for axis in axes:
        plot_tools.set_axes(axis, xlim=[1e10, 1e11], ylim=[0, 1], xscale='log',
                            xlabel=r'$M_\bigstar / \mathrm{M_\odot}$',
                            ylabel=r'$M_\mathrm{gas} / (M_\bigstar + M_\mathrm{gas})$', semilogx=True, which='major',
                            size=30)
        if axis != axis00:
    ...
    plt.savefig('/cosma7/data/dp004/dc-irod1/FLARES/spirals/plots/stellar_gas_mass1.png', bbox_inches='tight')
    plt.close()
